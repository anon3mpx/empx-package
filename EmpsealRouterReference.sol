...
contract EmpsealRouter is Maintainable, Recoverable, IEmpsealRouter {
    using SafeERC20 for IERC20;
    using OfferUtils for Offer;

    address public immutable WNATIVE;
    address public constant NATIVE = address(0);
    string public constant NAME = "EmpsealRouter";
    ...
    function _applyFee(uint256 _amountIn, uint256 _fee) internal view returns (uint256) {
        require(_fee >= MIN_FEE, "EmpsealRouter: Insufficient fee");
    ...
    function findBestPathWithGas(
        ...
        require(_maxSteps > 0 && _maxSteps < 5, "EmpsealRouter: Invalid max-steps");
    ...
    function _swapNoSplit(
        ...
        require(amounts[amounts.length - 1] >= _trade.amountOut, "EmpsealRouter: Insufficient output amount");
        for (uint256 i = 0; i < _trade.adapters.length; i++) {
    ...
    function swapNoSplitFromETH(
        ...
        require(_trade.path[0] == WNATIVE, "EmpsealRouter: Path needs to begin with WETH");
    ...
    function swapNoSplitToETH(
        ...
        require(_trade.path[_trade.path.length - 1] == WNATIVE, "EmpsealRouter: Path needs to end with WETH");
